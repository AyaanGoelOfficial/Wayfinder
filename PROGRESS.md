...
| 6 toll model and legality | **CLOSED.** Two mechanisms, both evidenced: EPE from Gazette S.O. 613(E) Table 5 at a rate fitted 10/10 against two boards, Yamuna verified at the mainline with ramp rates DEFERRED on one board. Plazas matched to interchanges by measured chainage, all three prior hand identifications reproduced from position alone. The three changes were isolated one at a time over one graph: the cost of driving is worth +161.8 tolled km, ramp attribution is worth 0.0 km and moves 0 of 56 routes (pure reclassification), the search proxy is worth -28.8 km and is the only one that lowers Yamuna. They do NOT compose: the Yamuna interaction term is -51.2 km, larger than any single effect. RESOLVED at gate 8: the 13.07 km northern approach is genuinely untolled, on local knowledge, and `gate:fixtures` now asserts Dankaur to Pari Chowk costs zero with a positive control beside it. The U-turn penalty has since landed, calibrated by sweep at `uTurnS` = 40 |
| 7 search and turn-by-turn | **CLOSED, and reviewed on the ground.** Search as you type over 7,675 places with a live latency readout and an index switch (warm medians 9 to 33 ms indexed against 36 to 62 unindexed, identical ranking enforced by test). Turn-by-turn from edge geometry, bearing deltas, road names and roundabout exit counting. A review by someone who drives `alpha-1 to surajpur` found five roundabouts where three were announced, a phantom turn, and a confusing enter/exit pair: all three had one root cause, two circles OSM never tagged, plus a presentation choice. Now 5 of 5 detected, both locally confirmed exit numbers preserved. Three toll display tiers driven by `tollDisplayOf`. Graph format v6 adds the road name table and the roundabout flag |
| 8 tracking | **CLOSED.** The blue dot, the chase camera, the maneuver banner and re-routing, built as a `TrackingEngine` in `shared/` that runs with no browser and no graph, so every scenario is a pure function of its trace. THE MATCHER GATES ON HEADING AND ONLY RANKS ON DISTANCE, because `npm run calibrate:tracking` measured that 71.9% of one-way road samples have an opposing carriageway inside `SNAP_TRACKING_M`, at 7.91 m separation at p5, so a corridor would have to be under 4 m to pick a side and no consumer fix is that good. Roundabouts excluded as a control, which moved p5 by 0.10 m. Free-drive matching is an HMM behind `POST /match`, server side because it needs the spatial index. `npm run verify:browser` EXISTS, 32 checks, driving headless Chrome over raw CDP with no puppeteer; its third result, NOTE, is allowed only with a per-run attribution argument printed beside the number. Throttled, as upper bounds from this machine: the matching itself (fix injection) at most 5.1 ms at 6x; the frame step's median 1.9 to 7.2 ms at 4x, nothing over 50 ms at 4x across four runs. Three defects were found by the gate and two more only by looking at a screenshot; all five are written up in `DESIGN.md` |
| 9 phone verification | NOT started. **HARD REQUIREMENT: measure the actual frame time on the actual device**, the tracking step's median against the 16.67 ms frame budget and every task over 50 ms, on a real mid-range phone. The 6x throttle on this laptop cannot answer it, see Known weaknesses. Also carries two on-the-ground checks nothing at a desk can do: read the POSTED car toll at the plaza against `TOLL_ROADS` in `config/city.ts` (the Jewar mainline fee, and a Yamuna RAMP board, which is the one rate still resting on a single observation), and judge whether class is standing in acceptably for road surface |

## Known weaknesses

- **THE 6x THROTTLE MEDIAN IS OPEN, and gate 9 answers it on a real phone.** Across nine runs the
  6x frame-step median read 5.9 to 16.2 ms against a 16.67 ms budget (upper bounds, this machine),
  so on a device that slow the tracking step may fill a frame. It now carries a verdict every run
  and has passed all four runs since, which does not close it. One new reading cuts the other way and is stated
  so it is not lost: within ONE run, two executions of the identical scripted drive gave 6x
  medians of 1.7 and 10.6 ms, so on this laptop the median is not a property of the code alone.
  Neither direction is settled here; the phone is the instrument.
- **THE FIRST 6x ATTRIBUTION ARGUMENT WAS INVALID, and it was corrected before commit.** It said
  the slow frames were not the matching path because none landed on a fix-injection frame. The
  injection sat OUTSIDE the timed region, so fix frames did no more timed work than any other and
  the pattern discriminated nothing. The replacement is a replay plus a garbage-collection trace
  with a live positive control, in `DESIGN.md`. Timed properly at last, the matching itself costs at
  most 5.1 ms at 6x. Slow frames at 6x now PASS, FAIL, or become a NOTE only when that run proves
  the frame fast on replay and free of collection.
- **`prefers-reduced-motion` is implemented but NOT verified under emulation.** The rule collapses
  durations and stops the rerouting spinner while leaving the glyph in place; that was confirmed
  by reading the stylesheet, not by rendering with the setting on. Untested is untested.
...
- **The places index EXISTS and is cross-validated: 7,675 entries.** `npm run gate:oracle` checks
  ...
  ranking itself was closed at gate 7.
