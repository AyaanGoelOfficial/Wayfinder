...
| 6 toll model and legality | **CLOSED.** Two mechanisms, both evidenced: EPE from Gazette S.O. 613(E) Table 5 at a rate fitted 10/10 against two boards, Yamuna verified at the mainline with ramp rates DEFERRED on one board. Plazas matched to interchanges by measured chainage, all three prior hand identifications reproduced from position alone. The three changes were isolated one at a time over one graph: the cost of driving is worth +161.8 tolled km, ramp attribution is worth 0.0 km and moves 0 of 56 routes (pure reclassification), the search proxy is worth -28.8 km and is the only one that lowers Yamuna. They do NOT compose: the Yamuna interaction term is -51.2 km, larger than any single effect. OPEN: 13.07 km of Yamuna mainline carries no toll tag, contiguous and bounded at an interchange, unresolvable from OSM alone. The U-turn penalty remains specified and unbuilt |
| 7 search and turn-by-turn | **BUILT, gates green, not yet accepted on local knowledge.** Search as you type over 7,675 places with a live latency readout and an index switch (warm medians 9 to 33 ms indexed against 36 to 62 unindexed, identical ranking enforced by test). Turn-by-turn from edge geometry, bearing deltas, road names and roundabout exit counting: 11 instructions for 73 km, 18 for 7.7 km. Three toll display tiers driven by `tollDisplayOf`. Graph format v6 adds the road name table and the roundabout flag. WAITING ON: roundabout exit numbers verified against roads the operator drives, and `verify:browser`, which is gate 8 |
